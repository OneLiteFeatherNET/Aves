...
class ItemFunctionTest {
    ...
    void testItemFunctionalInterface() {
        ItemFunction<IItem, Component> function  = item -> item == null ? null : item.get().getDisplayName();
        assertNull(function.apply(null));
        final IItem item = Item.of(ItemStack.builder(Material.ACACIA_FENCE).displayName(Component.text("Test")).build());
