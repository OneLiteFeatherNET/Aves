 ...
 * </p>
 * <p>
 * The stages of the Aves path are reading the palette into an array of state ids, building the
...
public class LightEngineStageBenchmark {
     ...
     */
    @Benchmark
    public SectionOpacity avesOpacity() {
     ...
     */
    @Benchmark
    public LightNibbles avesPropagate() {
     ...
     */
    @Benchmark
    public byte[] avesCollect() {
        return this.light.toDenseArray();
    }

    /**
     * Measures the whole Aves path so the sum of the stages can be checked against it.
     *
     * @return the calculated light array of the section
     */
    @Benchmark
    public byte[] avesFull() {
        int[] states = new int[BLOCK_COUNT];
        this.palette.getAll((x, y, z, value) -> states[(y << 8) | (z << 4) | x] = value);
