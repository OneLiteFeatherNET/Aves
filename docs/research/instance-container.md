...
Both `class AvesInstanceA extends InstanceContainer` and `class AvesInstanceB extends Instance`
...
```
[1] constructed custom Instance: avesprobe.ForeignInstance
...
If it is built, then as `AvesInstance extends Instance` in `net.theevilreaper.aves.instance`, **not**
