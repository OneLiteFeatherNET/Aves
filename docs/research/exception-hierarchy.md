...
**Question.** Replace the generic JDK exceptions in `net.theevilreaper.aves.instance.anvil` with a
...
**All sealed members must live in one package.** Aves has no `module-info.java`, so it runs in the
...
multi-catches in `AvesAnvilLoader`, and 14 `assertThrows(IOException.class)` assertions in the tests
...
1. **Exactly one translation point** from checked to unchecked: the catch in `AvesAnvilLoader.loadChunk`,
