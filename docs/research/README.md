...
Findings from three multi-agent investigations run while building the experimental Anvil chunk
loader. They are kept because each one answers a question that cost real effort to answer and that
will be asked again: *can we replace this part of Minestom, and is it worth it?*
...
- **`Light`**: possible, and a prototype was 3.1×–5.8× faster with bit-identical output — but the
  code path does not execute at all for the workload Aves actually has.

The pattern: *sealed-ness decides whether it is possible, and the profile decides whether it is
worth it.* Both have to be checked before designing anything, and neither can be guessed.
